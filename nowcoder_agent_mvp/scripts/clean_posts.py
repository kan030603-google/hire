...
def main() -> None:
    ...
    parser.add_argument("--expected-company-from-candidate", action="store_true")
    parser.add_argument("--config", type=Path, default=DEFAULT_COLLECTION_CONFIG)
    args = parser.parse_args()
    if not args.expected_company and not args.expected_company_from_candidate:
        parser.error("one of --expected-company or --expected-company-from-candidate is required")
        ...
        accepted.append(
            {
                **row,
                ...
                "detectedTitleCompanies": sorted(title_companies),
                "attributionStatus": (
                    "explicit-match" if expected_company in title_companies else "selector-only"
                ),
            }
        )
    ...
    summary = {
        ...
        "expectedCompany": "candidate.company" if args.expected_company_from_candidate else args.expected_company,
        "window": {
